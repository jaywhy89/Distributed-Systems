...
//private static final long serialVersionUID = 2123523532L;
public class MazeImpl extends Maze implements Serializable, ClientListener, Runnable {
        ...
        public synchronized boolean getprojectileMap(){
            return !projectileMap.isEmpty();
        }
        
        Collection deadPrj= new HashSet();  
        public synchronized boolean clientMoveProjectiles(Client client){
                if(!projectileMap.isEmpty()) {
                    System.out.println("size fo projectileMap is "+ projectileMap.size());
                    Iterator it = projectileMap.keySet().iterator();
                    synchronized(projectileMap) {
                            while(it.hasNext()) {   
                                    Object o = it.next();
       ...
       private synchronized Collection moveProjectile(Projectile prj) {
        ...
        private synchronized void killClient(Client source, Client target) {
                ...
                Direction d = Direction.notrandom();
               /* while(cell.isWall(d)) {
                        d = Direction.random();
                }*/
